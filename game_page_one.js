...
function main(){
    ...
    whistle.src="whistlee.mp3";
    
    crowd.src="crowd_sound.mp3";
    crowd.loop=true;

    goal_cel.src="Goal.mp3";
...
function movePlayer(event){
    ...
    else if(event.key==="ArrowUp" && player1Up){
        yPlayer1Vel=-6;
        player1Up=false;
    }
    else if(event.key===" "){
...
function frame(){
    ...
    if(Goal){
        ...
        if(xText>=1200){
            ...
            if(team1Goals==5){
                gameStarted=false;
                document.querySelector(".panel").textContent=`Player Wins ${team1Goals}:${team2Goals}. Press "Space" to restart`;
                ...
                gameStarted=false;
                document.querySelector(".panel").textContent=`CPU Wins ${team1Goals}:${team2Goals}. Press "Space" to restart`;
                document.querySelector(".panel").style.opacity="1.0";
                team2Goals=0;
                team1Goals=0;
                goal2.textContent=`${team2Goals}`;
...
let cntDown=0;
function control2(){
    const distx1=xBall+rBall-xPlayer1-rPlayer;
    const distx2=xBall+rBall-xPlayer2-rPlayer;

    if(player2Up && distx2<=0 && distx2>=-70 && (yBall<480) && (Math.abs(xBallVel)>10)){
        yPlayer2Vel=-6;
        xPlayer2Vel=-6;
        player2Up=false;
    }
    else if(player2Up && distx2<=0 && distx2>=-55 && (yBall<475)){
        yPlayer2Vel=-6;
        xPlayer2Vel=-6;
        player2Up=false;
    }
    else if(player2Up && distx2>0 && distx2<=40 && (yBall<475)){
        yPlayer2Vel=-6.5;
        xPlayer2Vel=4;
        player2Up=false;
    }
    else if(checker%6==0 && (xPlayer1>510 || (Math.abs(distx2)<=Math.abs(distx1)+Math.floor(Math.random()*(7))-3 && xBallVel<Math.abs(6)))){
        if(distx2<=-35){
            xPlayer2Vel=-4;
        }
        else{
    ...
    else if(checker%6==0 && xPlayer2<836){
        xPlayer2Vel=4
    }
    else if(checker%6==0 && xPlayer2>850){
        xPlayer2Vel=-4;
    }
    else if(checker%6==0){
        xPlayer2Vel=0;
    }
    if(player2Up && distx2>=0 && distx2<=55 && xPlayer2Vel>0){
        yPlayer2Vel=-6;
        player2Up=false;
    }

    const dist2y= yBall+rBall-yPlayer2-rPlayer;
    if(shoot && distx2>=-75 && distx2<=0 && Math.abs(dist2y)<100){
        shooting2=1;
        cnt++;
        cntDown=0;
    }
